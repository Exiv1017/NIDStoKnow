...
function useStudentAuthHeaders(userFromContext) {
  return useMemo(() => {
    // Prefer context
...
const StudentNotificationsBell = ({ refreshIntervalMs = 30000, appearance = 'light' }) => {
  const { user } = useContext(AuthContext);
  const headers = useStudentAuthHeaders(user);
  ...
  const bellDisabled = false; // allow opening even if not authenticated; actions will no-op
