...
import anom_what from '../../../content/modules/anomaly-based-detection/01-what-is-anomaly.md?raw';
import anom_how from '../../../content/modules/anomaly-based-detection/02-how-it-works.md?raw';
import anom_types from '../../../content/modules/anomaly-based-detection/03-types-of-anomalies.md?raw';
import anom_adv from '../../../content/modules/anomaly-based-detection/04-advantages.md?raw';
import anom_limits from '../../../content/modules/anomaly-based-detection/05-limitations.md?raw';
import anom_best from '../../../content/modules/anomaly-based-detection/06-best-practices.md?raw';
...
const anomalyBasedTheoryLessons = [
	{ title: 'What is Anomaly-Based Detection?', content: anom_what },
	{ title: 'How Anomaly-Based Detection Works', content: anom_how },
	{ title: 'Types of Anomalies', content: anom_types },
	{ title: 'Advantages of Anomaly-Based Detection', content: anom_adv },
	{ title: 'Limitations of Anomaly-Based Detection', content: anom_limits },
	{ title: 'Best Practices for Anomaly-Based Detection', content: anom_best },
];
...
const QuizBlock = ({ question }) => {
	...
	const handleSubmit = (e) => {
		e.preventDefault();
		setSubmitted(true);
		if (question.toLowerCase().includes('zero-day')) {
...
const AnomalyTheoryPage = () => {
	...
	const serverLessonProgress = useServerLessonProgress({ moduleTitle: 'Anomaly-Based Detection', lessons: anomalyBasedTheoryLessons, user });
	...
	useTimeSpentTracker({
		...
		lessonName: anomalyBasedTheoryLessons[currentLessonIdx]?.title || 'Theory',
		lessonsCompleted: currentLessonIdx + 1,
		totalLessons: anomalyBasedTheoryLessons.length,
	...
	// When local timer marks lesson as complete, persist to server.
	useEffect(()=>{
		if(!user?.id) return;
	...
	const questions = [
		{ id: 1, q: 'What is anomaly-based detection?', options: ['A', 'B', 'C'], answer: 0 },
			...
			<main className="flex-1 min-h-screen flex justify-center">
				<div className="max-w-7xl w-full p-6">
					<h1 className="text-2xl font-bold mb-4 text-gray-800 flex items-center gap-2">
						<span className="inline-block px-2 py-1 rounded text-blue-700 bg-blue-50">
							Anomaly-Based Detection{' '}
							<span className="text-base font-medium text-blue-400">Theory</span>
						</span>
					</h1>
					<div className="mb-6">
					...
					{!showAssessment ? (
						<div
							className="bg-white rounded-xl border border-gray-100 p-8 mb-8 shadow-sm w-full"
							style={{ minWidth: '0' }}
						>
						...
						>
							...
							<p className="mb-4 text-gray-700">
								Test your understanding of anomaly-based detection.
							</p>
...
export default AnomalyTheoryPage;
